'use strict';

let path = require('path');

let gulp;
let plugins;
let app;

module.exports = function ( _gulp, _plugins, _app ) {

    gulp = _gulp;
    plugins = _plugins;
    app = _app;

    return {
        /*
         * addNPMSupport
         * @param cb
         * <p>Delegates to separate uiFramework specific addFramework-Methods
         */
        'addNPMSupport' : function (cb) {
            // when uiFramework support is enabled
            if (app.wizard.uiFrameworkSupport) {
                // and uiFramework selected
                if (!app.fn.typechecks.isEmpty(app.wizard.uiFramework)) {
                    return app.fn.json.addNPMEntryToPackageConfiguration(
                        app.config.uiFrameworks[app.wizard.uiFramework][app.wizard.uiFrameworkVersion],
                        cb
                    );
                }
            }
            else {
                console.info('addNPMSupport skipped!');
            }
            cb();
        },


        /*
         * copyDependencies
         * @param cb
         * <p>Copies the uiFramework dependencies (e.g. preprocessor template).
         */
        'copyDependencies' : function (cb) {
            let src = null;
        // TODO!!!!!
            // when uiFramework support is enabled
            if (app.wizard.uiFrameworkSupport) {
                // check dependencies
                let jsonDependencies = app.fn.npm.getOwnPropertyValue(
                    app.config.uiFrameworks[app.wizard.uiFramework][app.wizard.uiFrameworkVersion],
                    'dependencies'
                );

                if (null !== jsonDependencies) {
                    for ( let jsonDependencyKey in jsonDependencies ) {
                        if ( jsonDependencyKey !== null
                                && jsonDependencies.hasOwnProperty(jsonDependencyKey) ) {

                            let jsonDependencyValue = app.fn.npm.getOwnPropertyValue(jsonDependencies, jsonDependencyKey);

                            if ( jsonDependencyKey === 'preprocessor' ) {
        // TODO!!!
        //                      copyPreprocessorTemplates(app.config.preprocessors[jsonDependencyValue], cb);
                            }
                        }
                    }
                }



                // // create uiFramework src path variable with user selected uiFramework informations
                // src = [
                //     path.join(
                //         app.config.paths.uiFrameworks,
                //         app.wizard.uiFramework,
                //         app.wizard.uiFrameworkVersion,
                //         app.config.paths.recursive
                //     )
                // ];
            }
            else {
                console.info('copyDependencies skipped!');
            }

            // return copyTemplatesSourcesToProjectFolder(src, null, cb);
            cb();
        },

        /*
         * copyTemplates
         * @param cb
         * <p>Copies the uiFramework template for the user selected uiFramework type to the default installation directory.
         */
        'copyTemplates' : function (cb) {
            let src = null;

            // when uiFramework support is enabled
            if (app.wizard.uiFrameworkSupport) {
                // create uiFramework src path variable with user selected uiFramework informations
                src = [
                    path.join(
                        app.config.paths.uiFrameworks,
                        app.wizard.uiFramework,
                        app.wizard.uiFrameworkVersion,
                        app.config.paths.recursive
                    )
                ];
            }
            else {
                console.info('copyTemplates skipped!');
            }

            return app.fn.template.copyTemplatesSourcesToProjectFolder(src, null, cb);
        }
    };
};
